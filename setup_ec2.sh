...
# 2. Python 패키지 설치: requirements.txt에 정의된 Python 라이브러리를 설치
# 3. Chrome 설치: 최신 버전의 Google Chrome을 설치
# 4. ChromeDriver 설치: Chrome 버전에 맞는 ChromeDriver를 다운로드하고 설치
# 5. 환경 변수 설정: .env 파일을 생성하여 데이터베이스 및 크롬 드라이버 경로를 설정
# 6. 크론잡 설정: 매일 자정에 main.py를 실행하도록 크론잡을 설정

# 1. 시스템 패키지 설치
echo "시스템 패키지 설치 중..."
sudo apt-get update
sudo apt-get install -y python3-pip unzip wget cron
